...
export class TestimonialEditComponent {
  ...
  private getTestimonialId(): void {
    ...
    if (!id) {
      this.alertService.toastError('Invalid testimonial ID.');
      this.goBack();
      return;
    }

    this.testimonialId = Number(id);

    if (!this.testimonialId || this.testimonialId <= 0) {
      this.alertService.toastError('Invalid testimonial ID.');
      this.goBack();
      return;
    }
  ...
  private loadTestimonial(): void {
    ...
    this.settingsService.getTestimonialById(this.testimonialId).subscribe({
      ...
      next: (response: any) => {
        ...
        if (!testimonial) {
          this.alertService.toastError('Testimonial not found.');
          this.goBack();
          return;
        }
        ...
        this.testimonialForm.patchValue({
          ...
          displayOrder: testimonial.displayOrder ?? testimonial.DisplayOrder ?? 1
        });
      ...
      error: (error) => {
        ...
        this.alertService.toastError(error?.error?.message ?? 'Failed to load testimonial.');
        this.goBack();
      }
  ...
  onSubmit(): void {
    ...
    this.alertService.loading('Updating testimonial...');

    this.settingsService.updateTestimonial(this.testimonialId, request).subscribe({

      next: (response: any) => {
        this.isSaving = false;
        this.alertService.close();
        this.alertService.toastSuccess(response?.message ?? 'Testimonial updated successfully.');
        this.goBack();
      },
      ...
      error: (error) => {
        ...
        const errorMessage = error?.error?.message || (typeof error?.error === 'string' ? error.error : null) || error?.error?.title || error?.message || 'Unable to update testimonial.';
