...
export class TermsConditionsComponent implements OnInit {
  ...
  loadTermsConditions(): void {
    ...
    this.settingsService.getTermsConditions().subscribe({
  ...
  update(): void {
    ...
    this.settingsService.updateTermsConditions({ content }).subscribe({

      next: (response) => {
        this.savedContent = content;
        this.isSaving = false;
        this.alertService.toastSuccess(response?.message ?? 'Terms & Conditions updated successfully.');
      },

      error: (error) => {
        console.error('Failed to update Terms & Conditions', error);
        this.isSaving = false;
        this.alertService.toastError(error?.error?.message ?? 'Failed to update Terms & Conditions.');
      }
